...
func (t *translator) mutexCount() int {
	...
	if mutexCount > maxMutexCount {
		mutexCount = maxMutexCount
	}
	return mutexCount
}
...
func (t *translator) addMutexProcess() {
	...
	// Transitions:
	// Idle, Write locked:
	trans1 := proc.AddTrans(idle, writeLocked)
	...
	trans2 := proc.AddTrans(writeLocked, idle)
	...
	trans5 := proc.AddTrans(readLocking, readLocked)
	...
	trans6 := proc.AddTrans(readLocked, idle)
	...
	trans6.AddUpdate("active_readers--")
	...
	// Read locked:
	trans7 := proc.AddTrans(readLocked, readLocked)
	...
	trans7.AddUpdate("active_readers++")
	...
	trans8 := proc.AddTrans(readLocked, readLocked)
	...
	trans8.AddUpdate("active_readers--")
	...
	trans9 := proc.AddTrans(readLocked, readToWriteLocked)
	...
	trans13 := proc.AddTrans(idle, bad)
	...
	trans14 := proc.AddTrans(readLocked, bad)
...
func (t *translator) addMutexProcessInstances() {
	...
	for i := 0; i < t.mutexCount(); i++ {
		...
		inst := t.system.AddProcessInstance(t.mutexProcess.Name(), instName)
